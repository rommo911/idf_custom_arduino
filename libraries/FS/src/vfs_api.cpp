...
{
    if(!_mountpoint) {
        ESP_LOGE(TAG,"File system is not mounted");
        return FileImplPtr();
    }

    if(!fpath || fpath[0] != '/') {
        ESP_LOGE(TAG,"%s does not start with /", fpath);
        return FileImplPtr();
    }
    ...
    if(!stat(temp, &st)) {
        ...
        ESP_LOGE(TAG,"%s has wrong mode 0x%08X", fpath, st.st_mode);
        return FileImplPtr();
    }
    ...
    if((mode && mode[0] != 'r') && create){
        ...
        while (token != NULL)
        {
            ...
            {
                ESP_LOGE(TAG,"Creating folder: %s failed!",folder);
                return FileImplPtr();
            }
    ...
    ESP_LOGE(TAG,"%s does not exist, no permits for creation", temp);
    free(temp);
    return FileImplPtr();
}
...
bool VFSImpl::rename(const char* pathFrom, const char* pathTo)
{
    ...
    if(!exists(pathFrom)) {
        ESP_LOGE(TAG,"%s does not exists", pathFrom);
        return false;
    }
...
bool VFSImpl::remove(const char* fpath)
{
    ...
    if(!f || f.isDirectory()) {
        ...
        ESP_LOGE(TAG,"%s does not exists or is directory", fpath);
        return false;
    }
    f.close();
...
bool VFSImpl::mkdir(const char *fpath)
{
    ...
    if(f && f.isDirectory()) {
        f.close();
        //ESP_LOGW(TAG,"%s already exists", fpath);
        return true;
    } else if(f) {
        f.close();
        ESP_LOGE(TAG,"%s is a file", fpath);
        return false;
    }
...
bool VFSImpl::rmdir(const char *fpath)
{
    ...
    if (strcmp(_mountpoint, "/spiffs") == 0) {
        ESP_LOGE(TAG,"rmdir is unnecessary in SPIFFS");
        return false;
    }
    ...
    if(!f || !f.isDirectory()) {
        ...
        ESP_LOGE(TAG,"%s does not exists or is a file", fpath);
        return false;
    }
    f.close();
    ...
    , _written(false)
{
    ...
    _path = strdup(fpath);
    if(!_path) {
        ESP_LOGE(TAG,"strdup(%s) failed", fpath);
        free(temp);
        return;
    }
    ...
    if(!stat(temp, &_stat)) {
        ...
        if (S_ISREG(_stat.st_mode)) {
            ...
            ESP_LOGE(TAG,"Unknown type 0x%08X for file %s", ((_stat.st_mode)&_IFMT), temp);
        }
    } else {
        //file not found
        if(!mode || mode[0] == 'r') {
            ...
            if(_d) {
                ...
                _isDirectory = false;
                ESP_LOGW(TAG,"stat(%s) failed", temp);
            }
        } else {
