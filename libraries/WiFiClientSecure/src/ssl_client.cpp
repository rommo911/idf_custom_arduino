...
static int _handle_error(int err, const char * function, int line)
{
    ...
    ESP_LOGE(TAG,"[%s():%d]: (%d) %s", function, line, err, error_buf);
#else
    ESP_LOGE(TAG,"[%s():%d]: code %d", function, line, err);
#endif
    return err;
}
...
int start_ssl_client(sslclient_context *ssl_client, const char *host, uint32_t port, int timeout, const char *rootCABuff, const char *cli_cert, const char *cli_key, const char *pskIdent, const char *psKey, bool insecure)
{
    ...
    int ret, flags;
    int enable = 1;
    ESP_LOGV(TAG,"Free internal heap before TLS %u", ESP.getFreeHeap());

    if (rootCABuff == NULL && pskIdent == NULL && psKey == NULL && !insecure) {
        return -1;
    }

    ESP_LOGV(TAG,"Starting socket");
    ssl_client->socket = -1;
    ...
    if (res < 0 && errno != EINPROGRESS) {
        ESP_LOGE(TAG,"connect on fd %d, errno: %d, \"%s\"", ssl_client->socket, errno, strerror(errno));
    ...
    if (res < 0) {
        ESP_LOGE(TAG,"select on fd %d, errno: %d, \"%s\"", ssl_client->socket, errno, strerror(errno));
        close(ssl_client->socket);
        return -1;
    } else if (res == 0) {
        ESP_LOGI(TAG,"select returned due to timeout %d ms for fd %d", timeout, ssl_client->socket);
        close(ssl_client->socket);
        return -1;
    } else {
        int sockerr;
        ...
        if (res < 0) {
            ESP_LOGE(TAG,"getsockopt on fd %d, errno: %d, \"%s\"", ssl_client->socket, errno, strerror(errno));
            close(ssl_client->socket);
            return -1;
        }

        if (sockerr != 0) {
            ESP_LOGE(TAG,"socket error on fd %d, errno: %d, \"%s\"", ssl_client->socket, sockerr, strerror(sockerr));
            close(ssl_client->socket);
            return -1;
        }
    }
...
#define ROE(x,msg) { if (((x)<0)) { ESP_LOGE(TAG,"LWIP Socket config of " msg " failed."); return -1; }}
    ...
    ESP_LOGV(TAG,"Setting up the SSL/TLS structure...");
    ...
    if (insecure) {
        ...
        ESP_LOGI(TAG,"WARNING: Skipping SSL Verification. INSECURE!");
    } else if (rootCABuff != NULL) {
        ESP_LOGV(TAG,"Loading CA cert");
        ...
        ESP_LOGV(TAG,"Setting up PSK");
        // convert PSK from hex to binary
        if ((strlen(psKey) & 1) != 0 || strlen(psKey) > 2*MBEDTLS_PSK_MAX_LEN) {
            ESP_LOGE(TAG,"pre-shared key not valid hex or too long");
            return -1;
        }
        ...
        if (ret != 0) {
            ESP_LOGE(TAG,"mbedtls_ssl_conf_psk returned %d", ret);
            return handle_error(ret);
        }
    } else {
        return -1;
    ...
    if (!insecure && cli_cert != NULL && cli_key != NULL) {
        ...
        ESP_LOGV(TAG,"Loading private key");
    ...
    ESP_LOGV(TAG,"Setting hostname for TLS session...");
    ...
    if (cli_cert != NULL && cli_key != NULL) {
        ESP_LOGD(TAG,"Protocol is %s Ciphersuite is %s", mbedtls_ssl_get_version(&ssl_client->ssl_ctx), mbedtls_ssl_get_ciphersuite(&ssl_client->ssl_ctx));
    ...
    ESP_LOGV(TAG,"Verifying peer X.509 certificate...");
    ...
    if ((flags = mbedtls_ssl_get_verify_result(&ssl_client->ssl_ctx)) != 0) {
        ...
        ESP_LOGV(TAG,"Certificate verified.");
    }
    ...
    ESP_LOGV(TAG,"Free internal heap after TLS %u", ESP.getFreeHeap());
...
{
    ESP_LOGV(TAG,"Cleaning SSL connection.");
...
int data_to_read(sslclient_context *ssl_client)
{
    ...
    //ESP_LOGE(TAG,"RET: %i",ret);   //for low level debug
    res = mbedtls_ssl_get_bytes_avail(&ssl_client->ssl_ctx);
    //ESP_LOGE(TAG,"RES: %i",res);    //for low level debug
...
{
    ESP_LOGV(TAG,"Writing HTTP request with %d bytes...", len); //for low level debug
    int ret = -1;

    while ((ret = mbedtls_ssl_write(&ssl_client->ssl_ctx, data, len)) <= 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE && ret < 0) {
            ESP_LOGV(TAG,"Handling error %d", ret); //for low level debug
            return handle_error(ret);
        }
...
{
    //ESP_LOGD(TAG, "Reading HTTP response...");   //for low level debug
    int ret = -1;

    ret = mbedtls_ssl_read(&ssl_client->ssl_ctx, data, length);

    //ESP_LOGV(TAG, "%d bytes read", ret);   //for low level debug
    return ret;
}
...
bool verify_ssl_fingerprint(sslclient_context *ssl_client, const char* fp, const char* domain_name)
{
    ...
    for (size_t i = 0; i < sizeof(fingerprint_local); ++i) {
        ...
        if (pos > len - 2) {
            ESP_LOGD(TAG,"pos:%d len:%d fingerprint too short", pos, len);
            return false;
        }
        uint8_t high, low;
        if (!parseHexNibble(fp[pos], &high) || !parseHexNibble(fp[pos+1], &low)) {
            ESP_LOGD(TAG,"pos:%d len:%d invalid hex sequence: %c%c", pos, len, fp[pos], fp[pos+1]);
            return false;
        }
        pos += 2;
    ...
    {
        ESP_LOGD(TAG,"fingerprint doesn't match");
        return false;
    }
...
{
    if (!ssl_client) {
        ESP_LOGD(TAG,"Invalid ssl_client pointer");
        return false;
    };

    const mbedtls_x509_crt* crt = mbedtls_ssl_get_peer_cert(&ssl_client->ssl_ctx);
    if (!crt) {
        ESP_LOGD(TAG,"Failed to get peer cert.");
        return false;
    };
...
{
    ESP_LOGD(TAG,"domain name: '%s'", (domain_name)?domain_name:"(null)");
    ...
    while (san != nullptr)
    {
        ...
        ESP_LOGD(TAG,"SAN '%s': no match", san_str.c_str());
    ...
    while (common_name != nullptr)
    {
        ...
        if (!MBEDTLS_OID_CMP(MBEDTLS_OID_AT_CN, &common_name->oid))
        {
            ...
            ESP_LOGD(TAG,"CN '%s': not match", common_name_str.c_str());
        }
