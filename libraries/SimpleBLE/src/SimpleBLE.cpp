...
static bool _init_gap(const char *name)
{
    ...
    {
        ESP_LOGE("simpleBLE","btStart failed");
        return false;
    }
    ...
    if (bt_state == ESP_BLUEDROID_STATUS_UNINITIALIZED)
    {
        ...
        {
            ESP_LOGE("simpleBLE","esp_bluedroid_init failed");
            return false;
        }
    }
    if (bt_state != ESP_BLUEDROID_STATUS_ENABLED)
    {
        ...
        {
            ESP_LOGE("simpleBLE","esp_bluedroid_enable failed");
            return false;
        }
    }
    if (esp_ble_gap_set_device_name(name))
    {
        ESP_LOGE("simpleBLE","gap_set_device_name failed");
        return false;
    }
    if (esp_ble_gap_config_adv_data(&_adv_config))
    {
        ESP_LOGE("simpleBLE","gap_config_adv_data failed");
        return false;
    }
    if (esp_ble_gap_register_callback(_on_gap))
    {
        ESP_LOGE("simpleBLE","gap_register_callback failed");
        return false;
    }
    return true;
}
...
bool SimpleBLE::begin(const String &localName)
{
    if (localName.length())
    {
