...
bool SPIFFSFS::begin(bool formatOnFail, const char * basePath, uint8_t maxOpenFiles, const char * partitionLabel)
{
    ...
    if(esp_spiffs_mounted(partitionLabel_)){
        ESP_LOGW("SPIFFS","SPIFFS Already Mounted!");
        return true;
    }
    ...
    if(err != ESP_OK){
        ESP_LOGE("SPIFFS","Mounting SPIFFS failed! Error: %d", err);
        return false;
    }
...
void SPIFFSFS::end()
{
    if(esp_spiffs_mounted(partitionLabel_)){
        ...
        if(err){
            ESP_LOGE("SPIFFS","Unmounting SPIFFS failed! Error: %d", err);
            return;
        }
...
bool SPIFFSFS::format()
{
    ...
    enableCore0WDT();
    if(err){
        ESP_LOGE("SPIFFS","Formatting SPIFFS failed! Error: %d", err);
        return false;
    }
    return true;
}
