...
bool BluetoothSerial::begin(const String &localName, bool isMaster)
{
    const std::string str = localName.c_str();
    return begin(str, isMaster);
}

bool BluetoothSerial::begin(const std::string &localName, bool isMaster)
{
    _isMaster = isMaster;
    if (localName.length())
    {
...
bool BluetoothSerial::connect(const String &remoteName)
{
    const std::string n = remoteName.c_str();
    return connect(n);
}

bool BluetoothSerial::connect(const std::string &remoteName)
{
