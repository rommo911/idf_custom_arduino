...
public:
    ...
    bool begin(const String &localName = String(), bool isMaster = false);
    bool begin(const std::string &localName = "", bool isMaster = false);
    bool begin(unsigned long baud)
    { // compatibility
        return begin(std::string(""));
    }
    int available(void);
    int peek(void);
    bool hasClient(void);
    ...
    bool connect(const std::string &remoteName);
