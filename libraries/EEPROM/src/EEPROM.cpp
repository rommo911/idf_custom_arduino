...
EEPROMClass::EEPROMClass(void)
    : _handle(0), _data(0), _size(0), _dirty(false), _name("eeprom"), _user_defined_size(0)
{
}
...
EEPROMClass::EEPROMClass(const char *name, uint32_t user_defined_size)
    : _handle(0), _data(0), _size(0), _dirty(false), _name(name), _user_defined_size(user_defined_size)
{
}
...
bool EEPROMClass::begin(size_t size)
{
  ...
  if (res != ESP_OK)
  {
    ESP_LOGE("EEPRPM","Unable to open NVS namespace: %d", res);
    return false;
  }
  ...
  {
    ESP_LOGE("EEPRPM","Unable to read NVS key: %d", res);
    return false;
  }
  if (size < key_size)
  { // truncate
    ESP_LOGW("EEPRPM","truncating EEPROM from %d to %d", key_size, size);
    uint8_t *key_data = (uint8_t *)malloc(key_size);
    if (!key_data)
    {
      ESP_LOGE("EEPRPM","Not enough memory to truncate EEPROM!");
      return false;
    }
  ...
  { // expand or new
    ...
    if (!expand_key)
    {
      ESP_LOGE("EEPRPM","Not enough memory to expand EEPROM!");
      return false;
    }
    // check for adequate free space
    if (nvs_set_blob(_handle, "expand", expand_key, expand_size))
    {
      ESP_LOGE("EEPRPM","Not enough space to expand EEPROM from %d to %d", key_size, size);
      free(expand_key);
      return false;
    }
    free(expand_key);
    ...
    if (key_size)
    {
      ESP_LOGI("EEPRPM","Expanding EEPROM from %d to %d", key_size, size);
    ...
    {
      ESP_LOGI("EEPRPM","New EEPROM of %d bytes", size);
    }
    nvs_commit(_handle);
    nvs_set_blob(_handle, _name, key_data, size);
    free(key_data);
  ...
  if (!_data)
  {
    ESP_LOGE("EEPRPM","Not enough memory for %d bytes in EEPROM", size);
    return false;
  }
  _size = size;
...
bool EEPROMClass::commit()
{
  ...
  {
    ESP_LOGE("EEPRPM","error in write");
  }
  else
  {
    _dirty = false;
...
uint16_t EEPROMClass::convert(bool clear, const char *EEPROMname, const char *nvsname)
{
  ...
  if (mypart == NULL)
  {
    ESP_LOGI("EEPRPM","EEPROM partition not found for conversion");
    return result;
  }
  ...
  if (!data)
  {
    ESP_LOGE("EEPRPM","Not enough memory to convert EEPROM!");
    goto exit;
  }

  if (esp_partition_read(mypart, 0, (void *)data, size) != ESP_OK)
  {
    ESP_LOGE("EEPRPM","Unable to read EEPROM partition");
    goto exit;
  }
  ...
  if (empty)
  {
    ESP_LOGI("EEPRPM","EEPROM partition is empty, will not convert");
    goto exit;
  }
  ...
  {
    ESP_LOGE("EEPRPM","Unable to open NVS");
    goto exit;
  }
  esp_err_t err;
  ...
  if (err != ESP_OK)
  {
    ESP_LOGE("EEPRPM","Unable to add EEPROM data to NVS: %s", esp_err_to_name(err));
    goto exit;
  }
  result = size;
  ...
  if (clear)
  {
    ...
    {
      ESP_LOGW("EEPRPM","Unable to clear EEPROM partition");
    }
  }
exit:
  free(data);
...
std::string EEPROMClass::read_string(int address)
{
...
size_t EEPROMClass::writeString(int address, const std::string &value)
{
  return EEPROMClass::writeString(address, value.c_str());
}
