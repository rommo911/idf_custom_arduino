...
std::string ArduinoOTAClass::getHostname()
{
    return _hostname;
}
...
std::string ArduinoOTAClass::getPartitionLabel()
{
...
std::string ArduinoOTAClass::readStringUntil(char end)
{
    std::string res = "";
    int value;
    while (true)
...
void ArduinoOTAClass::_onRx()
{
    if (_state == OTA_IDLE)
    {
        ...
        if (_password.length())
        {
            ...
            nonce_md5.begin();
            nonce_md5.add(std::to_string(micros()).c_str());
            nonce_md5.calculate();
            _nonce = nonce_md5.to_string();
    ...
    else if (_state == OTA_WAITAUTH)
    {
        ...
        _udp_ota.read();
        std::string cnonce = readStringUntil(' ');
        ...
        std::string challenge = _password + ":" + std::string(_nonce) + ":" + cnonce;
        MD5Builder _challengemd5;
        _challengemd5.begin();
        _challengemd5.add(challenge);
        _challengemd5.calculate();
        std::string result = _challengemd5.to_string();
