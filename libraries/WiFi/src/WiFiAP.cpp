...
bool WiFiAPClass::softAP(const char *ssid, const char *passphrase, int channel, int ssid_hidden, int max_connection, bool ftm_responder)
{
    ...
    if (err)
    {
        ESP_LOGE(TAG, "get AP config failed");
        return false;
    }
    if (!softap_config_equal(conf, conf_current))
    {
        ...
        if (err)
        {
            ESP_LOGE(TAG, "set AP config failed");
            return false;
        }
    }
 ...
 * @return SSID
 */
std::string WiFiAPClass::softAPSSID() const
{
    ...
    {
        return std::string(reinterpret_cast<const char *>(info.ap.ssid));
    }
    return std::string("");
}
 ...
 * @return String mac
 */
std::string WiFiAPClass::softAPmacAddress(void)
{
    uint8_t mac[6];
    char macStr[18] = {0};
    if (WiFiGenericClass::getMode() == WIFI_MODE_NULL)
...
const char *WiFiAPClass::softAPgetHostname()
{
    ...
    {
        ESP_LOGE(TAG, "Netif Get Hostname Failed!");
    }
    return hostname;
}
