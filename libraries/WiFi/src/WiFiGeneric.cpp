...
esp_err_t set_esp_interface_ip(esp_interface_t interface, IPAddress local_ip = IPAddress(), IPAddress gateway = IPAddress(), IPAddress subnet = IPAddress())
{
    ...
    if (interface != ESP_IF_WIFI_AP)
    {
        ...
        if (err)
        {
            ESP_LOGE(TAG, "DHCPC Get Status Failed! 0x%04x", err);
            return err;
        }
        err = esp_netif_dhcpc_stop(esp_netif);
        if (err && err != ESP_ERR_ESP_NETIF_DHCP_ALREADY_STOPPED)
        {
            ESP_LOGE(TAG, "DHCPC Stop Failed! 0x%04x", err);
            return err;
        }
        err = esp_netif_set_ip_info(esp_netif, &info);
        if (err)
        {
            ESP_LOGE(TAG, "Netif Set IP Failed! 0x%04x", err);
            return err;
        }
        if (info.ip.addr == 0)
        {
            ...
            if (err)
            {
                ESP_LOGE(TAG, "DHCPC Start Failed! 0x%04x", err);
                return err;
            }
        }
    }
    else
    {
        ...
        if (err)
        {
            ESP_LOGE(TAG, "DHCPS Get Status Failed! 0x%04x", err);
            return err;
        }
        err = esp_netif_dhcps_stop(esp_netif);
        if (err && err != ESP_ERR_ESP_NETIF_DHCP_ALREADY_STOPPED)
        {
            ESP_LOGE(TAG, "DHCPS Stop Failed! 0x%04x", err);
            return err;
        }
        err = esp_netif_set_ip_info(esp_netif, &info);
        if (err)
        ...
        if (err)
        {
            ESP_LOGE(TAG, "DHCPS Set Lease Failed! 0x%04x", err);
            return err;
        }
        ...
        if (err)
        {
            ESP_LOGE(TAG, "DHCPS Start Failed! 0x%04x", err);
            return err;
        }
    }
    return err;
...
esp_err_t set_esp_interface_dns(esp_interface_t interface, IPAddress main_dns = IPAddress(), IPAddress backup_dns = IPAddress(), IPAddress fallback_dns = IPAddress())
{
    ...
    if (interface != ESP_IF_WIFI_AP)
    {
        ...
        {
            ESP_LOGE(TAG, "Set Fallback DNS Failed!");
            return ESP_FAIL;
        }
    }
    return ESP_OK;
...
static inline const char* auth_mode_str(int authmode)
{
    switch (authmode)
    {
    case WIFI_AUTH_OPEN:
...
static inline char default_hostname[32] = {
    0,
};
static const char* get_esp_netif_hostname()
{
    if (default_hostname[0] == 0)
    {
        ...
        snprintf(default_hostname, 32, "%s%02X%02X%02X", "ESP-", eth_mac[3], eth_mac[4], eth_mac[5]);
    }
    return (const char*)default_hostname;
}
static inline void set_esp_netif_hostname(const char* name)
{
    if (name)
    {
...
static inline void _arduino_event_task(void* arg)
{
...
esp_err_t postArduinoEvent(arduino_event_t* data)
{
    ...
    if (event == NULL)
    {
        ESP_LOGE(TAG, "Arduino Event Malloc Failed!");
        return ESP_FAIL;
    }
    memcpy(event, data, sizeof(arduino_event_t));
    if (xQueueSend(_arduino_event_queue, &event, portMAX_DELAY) != pdPASS)
    {
        ESP_LOGE(TAG, "Arduino Event Send Failed!");
        return ESP_FAIL;
    }
    return ESP_OK;
}

static inline void _arduino_event_cb(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_AUTHMODE_CHANGE)
    {
        ...
        ESP_LOGV(TAG, "STA Auth Mode Changed: From: %s, To: %s", auth_mode_str(event->old_mode), auth_mode_str(event->new_mode));
    ...
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_CONNECTED)
    {
        ...
        ESP_LOGV(TAG, "STA Connected: SSID: %s, BSSID: " MACSTR ", Channel: %u, Auth: %s", event->ssid, MAC2STR(event->bssid), event->channel, auth_mode_str(event->authmode));
...
static inline bool _start_network_event_task()
{
    ...
    {
        xTaskCreatePinnedToCore(_arduino_event_task, "arduino_events", 4096, NULL, 2, &_arduino_event_task_handle, ARDUINO_EVENT_RUNNING_CORE);
        if (!_arduino_event_task_handle)
        {
...
void WiFiGenericClass::useStaticBuffers(bool bufferMode)
{
    if (lowLevelInitDone)
    {
        ESP_LOGW(TAG, "WiFi already started. Call WiFi.mode(WIFI_MODE_NULL) before setting Static Buffer Mode.");
    }
...
static inline bool espWiFiStart()
{
...
static inline bool espWiFiStop()
{
    esp_err_t err;
    if (!_esp_wifi_started)
    {
    ...
    if (err)
    {
        ESP_LOGE(TAG, "Could not stop WiFi! %d", err);
        _esp_wifi_started = true;
        return false;
    }
...
esp_err_t WiFiGenericClass::_eventCallback(arduino_event_t* event)
{
    ...
    {
        ESP_LOGD(TAG, "Arduino Event: %d - %s", event->event_id, arduino_event_names[event->event_id]);
    }
    ...
    else if (event->event_id == ARDUINO_EVENT_WIFI_STA_GOT_IP)
    {
        ...
        ESP_LOGD(TAG, "STA IP: %u.%u.%u.%u, MASK: %u.%u.%u.%u, GW: %u.%u.%u.%u",
            ip[0], ip[1], ip[2], ip[3],
    ...
    else if (event->event_id == ARDUINO_EVENT_ETH_GOT_IP)
    {
        ...
        uint8_t* mask = (uint8_t*)&(event->event_info.got_ip.ip_info.netmask.addr);
        uint8_t* gw = (uint8_t*)&(event->event_info.got_ip.ip_info.gw.addr);
        ESP_LOGD(TAG, "ETH IP: %u.%u.%u.%u, MASK: %u.%u.%u.%u, GW: %u.%u.%u.%u",
            ip[0], ip[1], ip[2], ip[3],
            mask[0], mask[1], mask[2], mask[3],
            gw[0], gw[1], gw[2], gw[3]);
#endif
...
bool WiFiGenericClass::mode(wifi_mode_t m)
{
    ...
    if (m & WIFI_MODE_STA)
    {
        ...
        if (err)
        {
            ESP_LOGE(TAG, "Could not set hostname! %d", err);
            return false;
        }
    }
    ...
    if (err)
    {
        ESP_LOGE(TAG, "Could not set mode! %d", err);
        return false;
    }
    if (_long_range)
    {
        if (m & WIFI_MODE_STA)
        {
            ...
            if (err != ESP_OK)
            {
                ESP_LOGE(TAG, "Could not enable long range on STA! %d", err);
                return false;
            }
        }
        if (m & WIFI_MODE_AP)
        {
            ...
            if (err != ESP_OK)
            {
                ESP_LOGE(TAG, "Could not enable long range on AP! %d", err);
                return false;
            }
        }
    }
...
bool WiFiGenericClass::initiateFTM(uint8_t frm_count, uint16_t burst_period, uint8_t channel, const uint8_t* mac)
{
    ...
    {
        ESP_LOGE(TAG, "Failed to initiate FTM session");
        return false;
    }
    return true;
}
 ...
 * @param callback_arg
 */
static inline void wifi_dns_found_callback(const char* name, const ip_addr_t* ipaddr, void* callback_arg)
{
    if (ipaddr)
    {
...
int WiFiGenericClass::hostByName(const char* aHostname, IPAddress& aResult)
{
    ...
    {
        ESP_LOGE(TAG, "DNS Failed for %s", aHostname);
    }
