...
private:
static constexpr char TAG[] = {"WiFiClientRxBuffer"};
...
int WiFiClient::setOption(int option, int *value)
{
    ...
    if(res < 0) {
        ESP_LOGE(TAG,"fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
    }
    return res;
}
    ...
    if (_connected) {
        ...
        if (res <= 0){
          switch (errno) {
              ...
              case ECONNABORTED:
                  _connected = false;
                  ESP_LOGD(TAG,"Disconnected: RES: %d, ERR: %d", res, errno);
                  break;
              default:
