...
void WiFiSTAClass::_setStatus(wl_status_t status)
{
    if (!_sta_status_group)
    {
        ...
        if (!_sta_status_group)
        {
            ESP_LOGE(TAG, "STA Status Group Create Failed!");
            _sta_status = status;
            return;
        }
    }
...
wl_status_t WiFiSTAClass::begin(const std::string& ssid, const std::string& passphrase, int32_t channel, const uint8_t* bssid, bool connect)
{
...
wl_status_t WiFiSTAClass::begin(const char* ssid, const char* passphrase, int32_t channel, const uint8_t* bssid, bool connect)
{
    ...
    {
        ESP_LOGE(TAG, "get current config failed!");
        return WL_CONNECT_FAILED;
    }
...
bool WiFiSTAClass::beginSmartConfig()
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "SmartConfig Set Type Failed!");
        return false;
    }
