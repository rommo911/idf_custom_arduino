...
static inline const char *cipher_str(int cipher)
{
    switch (cipher)
    {
 ...
 * @return (true if ok)
 */
bool WiFiScanClass::getNetworkInfo(uint8_t i, std::string &ssid, uint8_t &encType, int32_t &rssi, uint8_t *&bssid, int32_t &channel)
{
 ...
 */
std::string WiFiScanClass::SSID(uint8_t i)
{
    wifi_ap_record_t *it = reinterpret_cast<wifi_ap_record_t *>(_getScanInfoByIndex(i));
    if (!it)
    {
    ...
    return std::string(reinterpret_cast<const char*>(it->ssid));
}
 ...
 */
std::string WiFiScanClass::BSSIDstr(uint8_t i)
{
    char mac[18] = {0};
    wifi_ap_record_t *it = reinterpret_cast<wifi_ap_record_t *>(_getScanInfoByIndex(i));
    if (!it)
