 ...
 * @return String
 */
std::string HTTPClient::error_to_string(int error)
{
    switch (error)
    {
    case HTTPC_ERROR_CONNECTION_REFUSED:
    ...
    case HTTPC_ERROR_SEND_HEADER_FAILED:
        return std::string("send header failed");
    case HTTPC_ERROR_SEND_PAYLOAD_FAILED:
        return std::string("send payload failed");
    case HTTPC_ERROR_NOT_CONNECTED:
    ...
    case HTTPC_ERROR_NO_HTTP_SERVER:
        return std::string("no HTTP server");
    case HTTPC_ERROR_TOO_LESS_RAM:
        return std::string("too less ram");
    case HTTPC_ERROR_ENCODING:
        return std::string("Transfer-Encoding not supported");
    case HTTPC_ERROR_STREAM_WRITE:
        return std::string("Stream write error");
    case HTTPC_ERROR_READ_TIMEOUT:
        return std::string("read Timeout");
    default:
        return std::string("N/A");
    }
}

void HTTPClient::addHeader(const char *name, const char *value, bool first, bool replace)
{
    addHeader(String(name), String(value), first, replace);
}

void HTTPClient::addHeader(const std::string &name, const std::string &value, bool first, bool replace)
{
    addHeader(String(name.c_str()), String(value.c_str()), first, replace);
}
/**
...
void HTTPClient::addHeader(const String &name, const String &value, bool first, bool replace)
{
    ...
    const auto cond1 = std::string("Connection");
    const auto cond2 = std::string("User-Agent");
    const auto cond3 = std::string("Host");
    const auto cond4 = std::string("Authorization");
    /*  bool success = std::str::equals_ignore_case(name, cond1);
      success |= std::str::equals_ignore_case(name, cond2);
      success |= std::str::equals_ignore_case(name, cond3);
      success |= std::str::equals_ignore_case(name, cond4);

      if ( !success && _base64Authorization.length())*/
