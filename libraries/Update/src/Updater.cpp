...
bool UpdateClass::begin(size_t size, int command, int ledPin, uint8_t ledOn, const char *label)
{
    if (_size > 0)
    {
        ESP_LOGW("Update","already running");
        return false;
    }
    ...
    if (command == U_FLASH)
    {
        ...
        ESP_LOGD("Update","OTA Partition: %s", _partition->label);
    }
    ...
    else
    {
        ...
        ESP_LOGE("Update","bad command %u", command);
        return false;
    }
    ...
    {
        _error = UPDATE_ERROR_SIZE;
        ESP_LOGE("Update","too large %u > %u", size, _partition->size);
        return false;
    }
...
bool UpdateClass::end(bool evenIfRemaining)
{
    ...
    {
        ESP_LOGE("Update","premature end: res:%u, pos:%u/%u\n", getError(), progress(), _size);
        _abort(UPDATE_ERROR_ABORT);
        return false;
    }
